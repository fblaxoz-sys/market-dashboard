...
**As of 2026-07-22.** Check the git log before trusting this file.

## Shipped 2026-07-22 — weekly log, Excel export, trade notes (`69cfb07`, latest)

- **Weekly log view.** The log defaults to the current week (Monday 00:00 local)
  with a **This week / All time** toggle and a header showing the range and both
  counts. A *filter*, never a move — the full history is always intact, so a
  weekly reset can't lose anything.
- **Excel export replaces CSV.** Real `.xlsx`, scoped to the current view, sorted
  oldest-first. Written without any external library (store-only ZIP + minimal
  xlsx parts + a date number-format). Validated by decoding the generated bytes
  and opening them with `openpyxl` — CRCs valid, Date cells are true `datetime`s,
  order ascending, no timezone drift. See `ARCHITECTURE.md` § Trade log and
  `TESTING.md` §12.
- **Notes on trades.** Optional free text ("why I made this trade"), entered with
  the trade or added later via the Note cell → edit modal. Clearing removes it;
  notes flow into the export. Escaped on render (hostile HTML verified inert).

Earlier the same day: **`11699fd`** multi-account trade logging (an ACCOUNTS chip
row posts one trade into several accounts at once; sells skip accounts that don't
hold the ticker).

## Shipped 2026-07-21 — layout, columns, axes, 3M/6M
