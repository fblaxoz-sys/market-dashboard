...
| `doImport/exportTrades` | Bulk paste-in; **.xlsx** export (see below) |
| `weekStartTs/renderLogControls/setLogView` | Weekly vs all-time log scope |
| `crc32/zipStore/excelSerial/sheetXml/buildXlsx` | Self-contained xlsx writer |
...
every account a multi-account trade lands in), or added/edited later by clicking
the Note cell, which reuses the existing edit modal. An empty box deletes the key.
Free text from other users → escaped on render like everything else.

**xlsx export** (`buildXlsx`). The page ships no external libraries, so the
workbook is written directly rather than pulling in a CDN library:

- `zipStore()` — store-only (uncompressed) ZIP: local headers, central directory,
  EOCD, with a real `crc32()`. Uncompressed keeps it to ~80 lines with no deflate.
- Parts: `[Content_Types].xml`, `_rels/.rels`, `xl/workbook.xml`,
  `xl/_rels/workbook.xml.rels`, `xl/styles.xml`, `xl/worksheets/sheet1.xml`.
- Strings are **inline** (`t="inlineStr"`), avoiding a `sharedStrings` part.
- `excelSerial()` converts `ts` to a serial (days since 1899-12-30) built from the
  wall-clock parts via `Date.UTC`, so no timezone shift can move a trade to the
  previous day. `cellXfs[1]` applies built-in `numFmtId=22`, so the Date column
  arrives as a **true Excel date** — sortable/filterable, not text.
- Scope follows `logView`; rows are sorted **oldest first** so the sheet reads as a
  chronological record.

Verified by decoding the generated bytes and opening them with `openpyxl`: ZIP CRCs
valid, every Date cell parses as a `datetime`, order ascending, times preserved.
`TESTING.md` §12 has the procedure.

### Security note on rendering

Holdings, portfolio names and **trade notes** are written by **other users** and
rendered with `innerHTML`. Every interpolated value goes through `esc()`. Ticker
input is additionally restricted to `[A-Z0-9.\-^]`. This was tested with hostile
payloads in portfolio names, editor names and notes — see `TESTING.md`. If you add
a column, escape it.
